...
export class PaginationComponent implements OnInit, OnChanges {
    ...
    @Input()
    params: {recordCount: number, pageSize: number, shownPageCountMax: number, startPageNo: number, lang: string};
    ...
    shownPageCountMax  = 5; // 可显示的分页按钮数量最大值

    startPageNo  = 1; // 显示的分页按钮的开始页码

    endPageNo = 1; // 显示的分页按钮的结束页码

    selectedPageNo  = 1 ; // 当前选中的按钮页码
    ...
    previousButtonDesc: string;

    nextButtonDesc: string;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (changes['params'] && changes['params'].currentValue) {
            ...
            if (this.params.shownPageCountMax > 0 ) {
                const shownPageCountMax = this.params.shownPageCountMax;
            }
            ...
            if (this.params.lang === EN) {
                this.lang = EN;
                this.desc = `total:${this.recordCount}, page:${this.selectedPageNo} / ${this.pageCount}`;
                this.previousButtonDesc = 'previous';
                this.nextButtonDesc = 'next';
                this.zerodesc = 'total: 0';
            }else {
                this.lang = CHN;
                this.desc = `总计 ${this.recordCount} 条，第 ${this.selectedPageNo} / ${this.pageCount} 页`;
                this.previousButtonDesc = '上一页';
                this.nextButtonDesc = '下一页';
                this.zerodesc = '总计 0 条';
            }
